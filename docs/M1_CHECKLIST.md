...
최종 수정: 2026-07-29
목적: Cowork에서 작성한 M1 스켈레톤을 Godot 에디터에서 실행하고 튜닝하기 위한 절차와 검증 항목. 개발 환경은 Mac 기준 (2026-07-29 전환, docs/DECISIONS.md). 구 문서 docs/M1_WINDOWS_CHECKLIST.md는 폐기.
관련 문서: docs/PROTOTYPE.md (범위와 통과 기준), docs/ROOM_SPEC.md (방 규격), docs/HARNESS.md (검증 3계층), docs/MAC_SETUP.md (환경 셋업)

## 0. 전제

- 환경 설치가 끝나 있어야 한다. 아니라면 docs/MAC_SETUP.md를 먼저 1장부터 수행한다
- Godot 4.6.3-stable (버전 고정 근거는 docs/HARNESS.md)
- python tools/check.py 통과 확인됨. 엔진 로딩과 플레이는 미검증 상태다
...
3. 메인 씬을 실행한다 (Cmd+B). 메인 씬은 res://scenes/levels/stage_verify.tscn (검증용 방 2개)
4. 1막 러프 6방 구성은 res://scenes/levels/stage_act1_rough.tscn을 직접 열어 현재 씬 실행(Cmd+R)
5. 에디터 하단 gdUnit4 패널에서 tests/unit 전체를 실행해 5개 스위트가 통과하는지 확인한다
6. 첫 임포트로 생성된 산출물 중 커밋 대상(.uid)을 커밋한다
...
| 상태 창 | I | Back |
| 개발자 모드 켜기 끄기 | ` (백틱) | (미할당) |
| 스테이지 클리어 (개발자 모드) | P | (미할당) |
| 즉시 재시작 | F5 | Start |

- 사격은 바라보는 방향으로 나간다. W나 S 단독은 수직, 이동과 함께 누르면 대각 (S는 공중에서만). 탄창 소진 시 자동 재장전되며 재장전 중 이동이 감속된다 (2026-07-29 판정 반영)
- 게임패드 아날로그 스틱 조준은 미할당이다. 필요하면 에디터 입력 맵에서 추가한다
- 즉시 재시작 F5는 게임 내 debug_restart 액션이다. 에디터 실행 단축키(Cmd+B)와 별개다
...
| 대상 | 노드 / 파일 | 주요 파라미터 |
|---|---|---|
| 이동과 점프 | Player (scenes/player/player.tscn) | max_speed, ground_accel, air_accel, jump_height_tiles, time_to_peak, time_to_descent, jump_cut, coyote_time, jump_buffer, reload_move_multiplier |
...
| 총과 총검 | Player/Rifle | magazine_size, fire_interval, reload_time, auto_reload, bullet_damage, bullet_speed, melee_windup, melee_recovery |
...
| 카메라 | Stage | camera_limit_top, camera_limit_bottom (좌우는 방 단위 자동) |
